...
$expr = trim(htmlentities(mysqli_real_escape_string($dbc, $_POST['Expiration'])));
          ...
          if (intval($numInStock) >= intval($numberOfUnits))
          {
            ...
            if ($createOrderExecute)
            {
                ...
                if ($createShipmentExecute)
                {
                  ...
                  //email customer..doesn't work for nonlocal mail
                  mail($customerEmail, 'Order Confirmation', 'You\'re order has been received.', 'From: Admin@TruckingInc.com');
